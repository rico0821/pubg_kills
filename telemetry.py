...
class TeleProcessor:
	...
	def __init__(self, teleFile=None, teleData=None):
		
		if teleFile and teleData:
			print('ERROR')
			return 

		if teleFile:
			with open(teleFile) as f:
				self.telemetry = json.load(f)

		if teleData:
			self.telemetry = teleData
	...
	def getKillsXY(self, killer=None, victim=None):
		...
		kills = [[data['victim']['location']['x'], 
		          data['victim']['location']['y'],
		          data['killer']['location']['x'],
			  data['killer']['location']['y']] for data in kills_data]
